...
WITH RCT AS (
    ...
    FROM G20ACF9V.AMFLIBW.ITMRVAL0 ITMRVAL0
),
MRPRCT AS (
    SELECT 
        TRIM(ITMRVAL0.STIDAD) AS SITE,
...
TYPE1 AS (
    ...
    FROM RGNFILW.PSTBOMD PST
    WHERE 
        PST.PITTYP = 1 
        AND PST.BXDCOMPONENTITEMCLASS = 'RCT' AND BXDPARENTITEMCLASS IN ('ZACK','ZMLK','ZUCK','ZAMK','ZMMK','ZUMK')
...
SELECT DISTINCT 
    ...
    CASE WHEN T1.PCLASS IN ('ZMLK', 'ZMMK') THEN 'LEATHER' ELSE 'FABRIC' END AS STYLE
FROM TYPE1 T1
INNER JOIN RCT R 
    ON R.SITE = T1.WH 
