...
SELECT [work_day]
    ...
    , MAX(t1.End_Time) as [End_Time]  --accounting for cases where the shift is yet to complete by Crystal
    , MAX(t1.[End Time])      [End Time] --  by Jim,Shen on Aug.29.2024
...
(
    SELECT DISTINCT
         ...
         , t.[description]
         , t.clock_in
        , (t.[End Time]) [End_Time]   -- by crystal
        , DATEADD(MINUTE, 
                    DATEDIFF(MINUTE, '00:00:00', CAST(t.[End Time] AS TIME)) / 15 * 15,
                    CAST(CAST(t.[End Time] AS DATE) AS DATETIME)
                    ) AS [End Time] -- Aug.29.2024 - to round the clock out time to the nearest 15 minutes by Jim,Shen 
        , CAST(DATEDIFF(SECOND, t.clock_in, DATEADD(MINUTE, 
            DATEDIFF(MINUTE, '00:00:00', CAST(t.[End Time] AS TIME)) / 15 * 15,
            CAST(CAST(t.[End Time] AS DATE) AS DATETIME))) / 60.00 AS FLOAT) AS spent_time_minutes    -- -- Aug.29.2024 - to round the clock out time to the nearest 15 minutes by Jim,Shen 
    FROM
    (
       ...
       SELECT [home_wh_id]
        ...
        FROM [PowerBI_Distribution].[EmployeeClockInOut]  a
            LEFT JOIN [PowerBI_Distribution].[Employee]    b
