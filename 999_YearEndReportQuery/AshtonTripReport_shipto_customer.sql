...
trip_report AS
(SELECT *, CAST(LEFT(t.LoadID, 7) AS INT) AS trip_nbr  FROM Distribution_Warehouse_Wholesale.TripReport  as t  WHERE t.WhID = '335' AND t.TripStatus NOT IN ('S','X')
),
customer as
...
SELECT t1.WhID,
	...
	CASE
        ...
        WHEN t6.LoadablePct >= 90 AND t6.LoadablePct <= 100 THEN '90% ~ 100%'
