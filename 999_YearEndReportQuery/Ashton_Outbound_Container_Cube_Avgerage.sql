...
FROM (select t.start_tran_date,
       ...
       t.routing_code,
       t.tran_qty
      from Distribution_Warehouse_Wholesale.TranLog as t where t.wh_id IN ('335')  AND t.start_tran_date >= '2024-06-01'
	  AND t.tran_type IN ('347'))  AS t1
...
SELECT  
    ...
    a2.Container_Type,        
    SUM(CASE WHEN a1.row_num_shipping = 1 then 1 else 0 end) as Container_Count,         
    CAST(SUM(a1.Shipped_cubes) AS INT) as shipped_cubes,         
    CAST(SUM(a1.Shipped_cubes)/SUM(CASE WHEN a1.row_num_shipping = 1 then 1 else 0 end) AS INT) as Avg_Cube_per_Container       
FROM trx as a1
LEFT JOIN ContainerType as a2 ON a1.Container_nbr = a2.Container_nbr
GROUP BY a1.YearMonth,
