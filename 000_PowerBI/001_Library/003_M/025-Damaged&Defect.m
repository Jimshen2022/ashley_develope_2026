let
    Source = Sql.Database("ashley-edw.database.windows.net", "ASHLEY_EDW", [Query="-- Aug.16.2024 Updated Ashton damaged and defect products BI report by new process -- JimShen
...
FROM Distribution_Warehouse_Wholesale.TranLog AS t1
WHERE t1.wh_id IN ('335') AND t1.start_tran_date > '2023-01-01' AND t1.lot_number is not null and len(t1.lot_number)>5
	AND t1.location_id_2 IN ('DM001AA1','NG001MT1','VD001AA1','EX001AA1','SH001AA1','NG001CK3','NG001CG3','NG001UP3','NG001VD3','DR001AA1','DR001AB1') 
...
WHERE  t1.wh_id  IN ('335') 
	...
	AND t1.serial_no_status NOT IN ('O') and t1.master_status NOT IN ('S') ", CommandTimeout=#duration(0, 1, 0, 0)]),
    #"Changed Type" = Table.TransformColumnTypes(Source,{{"received_into_ashton_date", type date}, {"received_into_loc_date", type date}}),
    #"Added Conditional Column" = Table.AddColumn(#"Changed Type", "PendingDays", 
    
    each 
        if [received_into_loc_date] is null then Date.From(DateTime.LocalNow()) - [received_into_ashton_date]
        else if  [location_id] = {"DM001AA1","NG001MT1"} then Date.From(DateTime.LocalNow()) - [received_into_ashton_date]
        else Date.From(DateTime.LocalNow()) - [received_into_loc_date] 
    ),
    #"Changed Type1" = Table.TransformColumnTypes(#"Added Conditional Column",{{"PendingDays", Int64.Type}})
in
    #"Changed Type1"