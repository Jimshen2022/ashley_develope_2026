 ...
 *    System   			: HighJump  		 							
 *    Procedure			: usp_ww_active_container_information			  
 *    Date				: 06/05/2025 
 *	  Version			: 2.0							
 *    Description		: This procedure is used for page 2061 Active Container Information
 *    Modification Log  : Date 			Modified By 		Description
 *						  06/20/2025 	Leo Le				Created
 *						  06/30/2025 	Leo Le				V1.1 add Distinct
 *						  07/21/2025 	Leo Le				V2  fillter by TRSFWH
 *						  09/22/2025 	Leo Le				V3  add new column Week 1, 2 WW-87
 
 -- Test script for [usp_ww_active_container_information]

DECLARE 
	@WhID NVARCHAR(10) = '1',
	@LoadID NVARCHAR(30) = '%',
	@EquipmentID NVARCHAR(20) = ''
EXEC dbo.usp_ww_active_container_information
	  @in_vchWhID = @WhID
	, @in_vch_Load_ID = @LoadID
	, @in_equipment_id = @EquipmentID


 ********************************************************************************/

 
CREATE     PROCEDURE [dbo].[usp_ww_active_container_information]  
...
BEGIN
    ...
    SELECT 
        ldm.load_id,
        ldm.equipment_id,
	    pkd.item_count as item_count, -- add loaded item column
        CASE
            WHEN EXISTS (
                SELECT TOP 1 1
        ...
        ldm.door_loc,
        ldm.transfer_wh_id,
        --LEO LE 09/22/2025 V3 using only load id 
		--TRSFWH.TRSFWHName as transfer_wh_id,
        --LEO LE 09/22/2025 V3 show only created date
        CONVERT(VARCHAR(10), ldm.trip_create_date, 23) AS create_date,
        ldm.wh_id,
        trl.location_id,
        ldm.status

    INTO #t_active_container
    ...
    LEFT JOIN (
        SELECT 
            load_id,
            pkd.wh_id,
		    COUNT(DISTINCT pkd.item_number) AS item_count, -- add loaded item column
 ...
 ---- V2
	LEFT JOIN 
	(select text as TRSFWHID,description+'('+ text + ')' as TRSFWHName, wh_id from t_lookup (nolock) 
...
----V2 END
    WHERE 
		...
		AND ldm.transfer_wh_id like @in_vch_Load_ID -- V2
        AND ldm.equipment_id LIKE @in_equipment_id + '%' 
    
    --LEO LE 09/22/2025 V3 Start
    SELECT 
    equipment_id,
    ...
    INTO #t_active_order
        FROM (
            ...
            JOIN t_load_master (NOLOCK) ldm 
                ON hot.wh_id = ldm.wh_id AND hot.load_id = ldm.load_id
            JOIN t_order (NOLOCK) orm 
                ON ldm.wh_id = orm.wh_id AND hot.order_number = orm.order_number
            WHERE 
                ldm.status <> 'S'
                AND ldm.load_type = 'H'
                AND hot.stage_loc <> ''
      ...
      --V3 Start END
	    ...
	    SELECT 
        ----CASE WHEN special_ship_inst = 'Y' THEN '{{BGCOLOR=#FFFF33}}' END AS color, 
        ------Commented to make sure the report is rendering correct data with Shipping Instruction
        Distinct 
        CASE
    ...
    DROP TABLE #t_active_container
    DROP TABLE #t_active_order
END
