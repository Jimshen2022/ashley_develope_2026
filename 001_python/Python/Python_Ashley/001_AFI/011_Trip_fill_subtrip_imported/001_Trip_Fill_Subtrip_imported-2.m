...
sql_parts.append('    CAST(CAST(t1.start_tran_date AS DATETIME) + CAST(t1.start_tran_time AS DATETIME) AS DATETIME) AS start_tran_datetime')
...
"
    ...
    #"Filtered Rows" = Table.SelectRows(Source, each ([Name] = "result")),
    #"Expanded Value" = Table.ExpandTableColumn(#"Filtered Rows", "Value", {"tran_type", "description", "employee_id", "control_number_2", "start_tran_date", "start_tran_time", "start_tran_datetime", "tran_qty", "imported", "trip_nbr", "trip_nbr_2", "time_diff_hours", "time_diff_range"}, {"tran_type", "description", "employee_id", "control_number_2", "start_tran_date", "start_tran_time", "start_tran_datetime", "tran_qty", "imported", "trip_nbr", "trip_nbr_2", "time_diff_hours", "time_diff_range"}),
    #"Reordered Columns" = Table.ReorderColumns(#"Expanded Value",{"Name", "tran_type", "description", "employee_id", "control_number_2", "start_tran_date", "start_tran_time", "tran_qty", "start_tran_datetime", "imported", "trip_nbr", "trip_nbr_2", "time_diff_hours", "time_diff_range"}),
    #"Changed Type" = Table.TransformColumnTypes(#"Reordered Columns",{{"time_diff_hours", type number}, {"trip_nbr", Int64.Type}, {"imported", type datetime}, {"start_tran_datetime", type datetime}, {"tran_qty", Int64.Type}, {"start_tran_date", type date}, {"employee_id", type text}}),
    #"Removed Columns" = Table.RemoveColumns(#"Changed Type",{"Name"}),
    #"Changed Type1" = Table.TransformColumnTypes(#"Removed Columns",{{"time_diff_hours", type number}}),
    #"Renamed Columns" = Table.RenameColumns(#"Changed Type1",{{"start_tran_datetime", "first_fill_request_time"}, {"imported", "sub_trip_imported_time"}})
in
    #"Renamed Columns"