...
BEGIN
    ...
    BEGIN
        SELECT
            d.wh_id,
            CONVERT(CHAR(10), d.dispatch_date, 111) + ' '
                + CONVERT(CHAR(8), d.dispatch_time, 108)  AS dispatch_date,
            d.item_number,
            d.trip_number,
            CAST(LEFT(d.trip_number, CHARINDEX('-', d.trip_number) - 1) AS INT) AS trip_nbr,
            d.ldm_status,
            d.trip_needed,
            d.trip_picked,
            d.available_sto,
            itm.pick_put_id,
            CASE WHEN itm.pick_put_id = 'UPH' THEN 'UPH' ELSE 'CG' END AS product_category,
            CASE
                WHEN (
                    SELECT COUNT(DISTINCT CASE WHEN itm2.pick_put_id = 'UPH' THEN 'UPH' ELSE 'CG' END)
                    FROM #temp_demand d2
                    LEFT JOIN t_item_master itm2 (NOLOCK)
                        ON itm2.item_number = d2.item_number
                        AND itm2.wh_id = d2.wh_id
                    WHERE CAST(LEFT(d2.trip_number, CHARINDEX('-', d2.trip_number) - 1) AS INT)
                        = CAST(LEFT(d.trip_number,  CHARINDEX('-', d.trip_number)  - 1) AS INT)
                ) = 1
                THEN (
                    SELECT DISTINCT CASE WHEN itm2.pick_put_id = 'UPH' THEN 'UPH' ELSE 'CG' END
                    FROM #temp_demand d2
                    LEFT JOIN t_item_master itm2 (NOLOCK)
                        ON itm2.item_number = d2.item_number
                        AND itm2.wh_id = d2.wh_id
        ...
        LEFT JOIN t_item_master itm (NOLOCK)
            ON itm.item_number = d.item_number
            AND itm.wh_id = d.wh_id
        order BY d.dispatch_date, d.dispatch_time, trip_nbr, item_number
    END
    ELSE  /* 汇总报表 */
    BEGIN
        ...
        FROM #temp_demand m
        LEFT JOIN t_item_master itm (NOLOCK)
            ON itm.item_number = m.item_number
            AND itm.wh_id = m.wh_id
        GROUP BY
