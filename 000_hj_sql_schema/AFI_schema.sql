...
SELECT  *  FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME LIKE '%import%'
...
SELECT 
    COLUMN_NAME,
    DATA_TYPE,
    CHARACTER_MAXIMUM_LENGTH,   -- 字符数上限（varchar/nvarchar）
    CHARACTER_OCTET_LENGTH,     -- 字节数上限
    NUMERIC_PRECISION,
    NUMERIC_SCALE
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_NAME = 't_order_c_number'
-- AND COLUMN_NAME = '字段名'  -- 可加这行只看某个字段
...
-- sn trx
select * from t_tran_log where lot_number IN ('503953343055') order by item_number, lot_number, start_tran_date desc, start_tran_time desc
