...
# ==============================================================================
# STEP 3: CLEAN UP OLD FILES (OLDER THAN 1 YEAR / 365 DAYS)
# ==============================================================================
Write-Host "Checking for old files (older than 1 year) in the folder..."
$limitDate = (Get-Date).AddDays(-365)

# Retrieve all Excel files (.xls, .xlsx, .xlsb) excluding the original template file
$files = Get-ChildItem -Path $folderPath -Include "*.xls*", "*.xlsb" -Recurse | Where-Object {
    $_.Name -ne $templateName -and $_.PSIsContainer -eq $false
}

foreach ($file in $files) {
    # Check the creation date (CreationTime) of each file
    if ($file.CreationTime -lt $limitDate) {
        try {
            Write-Host "Expired file detected: $($file.Name) (Created on: $($file.CreationTime)). Deleting..."
            Remove-Item -Path $file.FullName -Force
        }
        catch {
            Write-Error "Failed to delete file $($file.Name): $_"
        }
    }
}
