...
WHERE 
a.WCHPOSTEDTIMESTAMP BETWEEN CONVERT(VARCHAR(10), DATEADD(DAY, -14, GETDATE()), 120) AND CONVERT(VARCHAR(10), GETDATE(), 120)
...
AND a.WCILASTMAINTENANCETIMESTAMP BETWEEN CONVERT(VARCHAR(10), DATEADD(DAY, -7, GETDATE()), 120) AND CONVERT(VARCHAR(10), GETDATE(), 120)
) 
UNION ALL
(SELECT LTRIM(RTRIM(a.WCICONTAINERNUMBER)) as ContainerNumber, a.WCIORIGIN, a.WCIDESTINATION, a.WCIORDER, LTRIM(RTRIM(a.WCIITEMNUMBER)) as ItemNumber, a.WCIQUANTITYLOADED as Qty, 
a.WCILASTMAINTENANCETIMESTAMP, a.WCILASTMAINTENANCEUSER,
...
WHERE n.WCHBUILDING LIKE 'B%'
    and b.Product not in ('CG','Bedding')
...
csv_path = os.path.join(output_dir, f"raw_mil_loading_pieces_{current_time}.csv")
