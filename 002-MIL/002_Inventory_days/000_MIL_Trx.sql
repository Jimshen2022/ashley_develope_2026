SELECT
    ...
    -- 拼接日期和时间为时间戳格式（SQL Server 中用 CONCAT 和 RIGHT）
TRY_CAST(
    CONCAT(
        '20',
        SUBSTRING(CAST(t1.UPDDT AS VARCHAR(8)), 2, 2),  -- 年
        '-',
        SUBSTRING(CAST(t1.UPDDT AS VARCHAR(8)), 4, 2),  -- 月
        '-',
        SUBSTRING(CAST(t1.UPDDT AS VARCHAR(8)), 6, 2),  -- 日
        ' ',
...
BETWEEN
    -- ✅ 四周前的周日（含）
    DATEADD(DAY, -28, DATEADD(DAY, 1 - DATEPART(WEEKDAY, GETDATE()), GETDATE()))
    AND
    -- ✅ 上周六的 23:59:59
    DATEADD(SECOND, -1, DATEADD(DAY, 1 - DATEPART(WEEKDAY, GETDATE()), GETDATE()))
