...
WITH MaxEnteredYard AS (
    ...
    WHERE
        a.wh_id = '31' AND a.entered_yard >= @load_date
    GROUP BY
...
CTN AS (
    SELECT
        ...
        END AS ctn_size
    FROM
        (select * from Distribution_Warehouse_Wholesale.Trailer as a0 WHERE a0.wh_id = '31' AND a0.entered_yard >= @load_date) AS a
    INNER JOIN MaxEnteredYard AS mey ON a.area_id = mey.wh_id AND a.equipment_id = mey.equipment_id AND a.entered_yard = mey.max_entered_yard
),
ItemInfo AS (
    SELECT
...
ldm as (SELECT t1.wh_id
             ...
             , t1.trip_create_date
             , t1.transfer_wh_id
             , max(t1.loadDate) load_date_2
        FROM Distribution_Warehouse_Wholesale.LoadMaster AS T1
        WHERE T1.wh_id IN ('35', '34', '33', '31')
        GROUP BY
               t1.wh_id
             , t1.load_id
             , t1.door_loc
...
tripDetail as (
    select * from Distribution_Warehouse_Wholesale.TranLog as t0
        where t0.wh_id in ('35', '31', '33', '34')
            and t0.tran_type in ('361')
            and t0.start_tran_date between '2024-01-01' AND getdate()
),
TripInfo AS (
    SELECT DISTINCT
        td.wh_id,
    ...
    FROM
        tripDetail as td
    LEFT JOIN CTN AS ctn ON ctn.equipment_id = td.routing_code AND ctn.wh_id = td.wh_id
),
MixedContainer AS (
    SELECT
        b1.wh_id,
        ...
        CASE WHEN COUNT(DISTINCT b1.product) = 1 THEN  max(b1.product) ELSE 'Mixed' END AS container_category
    FROM (
        SELECT
            td.wh_id,
            td.item_number,
...
SELECT
    ...
    sum(itm.unit_cube * t1.tran_qty) AS cubes,
    CAST(
        sum(itm.unit_cube * t1.tran_qty) /
        CASE
    ...
    CASE
        WHEN t1.wh_id IN ('31') THEN 'WN1'
        WHEN t1.wh_id IN ('33') THEN 'WN2'
        WHEN t1.wh_id IN ('35') AND SUBSTRING(ldm.door_loc,1,2) IN ('D1','D2') THEN 'WN2'
        WHEN t1.wh_id IN ('35') AND SUBSTRING(ldm.door_loc,1,2) IN ('D8','D9') THEN 'MPT'
        WHEN t1.wh_id IN ('35') AND SUBSTRING(ldm.door_loc,1,2) IN ('D4') THEN 'WN3'
...
LEFT JOIN ldm as ldm ON t1.wh_id = ldm.wh_id AND t1.control_number_2 = ldm.load_id AND t1.routing_code = ldm.equipment_id
GROUP BY
    t1.tran_type,
    t1.wh_id,
    t1.routing_code,
