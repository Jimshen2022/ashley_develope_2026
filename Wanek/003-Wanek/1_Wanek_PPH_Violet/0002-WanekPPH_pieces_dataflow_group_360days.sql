...
SELECT d.item_number, -- ok
       ...
       d.location_id_2,
       d.hu_id,
       d.return_disposition as backorder_reason,
       sum(d.tran_qty) as tran_qty -- ok
FROM trx_with_sn as d
group by d.item_number, -- ok
       d.wh_id, -- ok
       d.tran_type, -- ok
       d.shift_date,-- ok
       d.shift,-- ok
