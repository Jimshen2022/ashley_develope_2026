...
WITH ContainerDetails AS (
    ...
    JOIN (select itnbr, itcls, B2Z95S, WEGHT, stid from MasterData_ItemMaster_WNK.ITMRVA where stid = '35') as  c ON a.WCIITEMNUMBER = c.itnbr 
...
SELECT 
    ...
    -- Shift 列
    CASE 
        WHEN CAST(h.WCHPOSTEDTIMESTAMP AS TIME) >= '07:00:00' 
         AND CAST(h.WCHPOSTEDTIMESTAMP AS TIME) < '19:00:00' THEN 'D'
        ELSE 'N'
    END AS Shift,
    -- Shift_Date 列
    CASE 
        WHEN CAST(h.WCHPOSTEDTIMESTAMP AS TIME) >= '07:00:00' 
         AND CAST(h.WCHPOSTEDTIMESTAMP AS TIME) < '19:00:00' 
        THEN CAST(h.WCHPOSTEDTIMESTAMP AS DATE)
        ELSE 
            CASE 
                WHEN CAST(h.WCHPOSTEDTIMESTAMP AS TIME) >= '19:00:00' 
                THEN CAST(h.WCHPOSTEDTIMESTAMP AS DATE)
                ELSE DATEADD(DAY, -1, CAST(h.WCHPOSTEDTIMESTAMP AS DATE))
            END
    END AS Shift_Date,
    CASE 
...
WHERE d.[Container#] IS NOT NULL and d.[Container#] = '33-AXIU1616126-5-2025-02-04 00'
