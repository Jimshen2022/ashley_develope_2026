WITH ContainerDetails AS (
    ...
    WHERE a.WCIORIGIN IN ('35','33')
      AND a.WCILASTMAINTENANCETIMESTAMP BETWEEN DATEADD(DAY, -65, CAST(GETDATE() AS DATE)) AND CAST(GETDATE() AS DATE)
      AND SUBSTRING(LTRIM(RTRIM(a.WCICONTAINERNUMBER)), 1, 4) NOT IN ('AAAR', 'AIIR', 'AAIR', 'AIRR', 'AIR_', 'AIR1', 'AAII', 'ARRR')
    ...
    SELECT 
        ...
        a.WCIQUANTITYLOADED * c.B2Z95S AS Cubes,
        CEILING(CAST(a.WCIQUANTITYLOADED AS FLOAT) / b.ITMCQTY) AS Cartons,
        LTRIM(RTRIM(a.WCIORIGIN)) + '-' + LTRIM(RTRIM(a.WCICONTAINERNUMBER)) + '-' + LTRIM(RTRIM(a.WCIDESTINATION)) + '-' + LEFT(CONVERT(VARCHAR(23), a.WCIARCHIVETIMESTAMP, 121), 13) AS [Container#],
        CASE 
            WHEN a.WCIITEMNUMBER LIKE 'B%' THEN 'CG'
            WHEN c.ITCLS NOT LIKE 'Z%' THEN 'RP'
            WHEN c.ITCLS LIKE 'Z%' AND c.ITCLS LIKE '%K' THEN 'Un-Kits'
...
HeaderDetails AS (
    ...
    WHERE a.WCHCONTAINERSTATUS IN ('P', 'T')
      ...
      OR (a.WCHACTUALARRIVALMAINTPROGRAM NOT IN ('SVCHECKIN') AND a.WCHBUILDING IN ('B1','B2','V3','M3','K1','33')))
      AND a.WCHBUILDING NOT IN ('B5')
      AND a.WCHPOSTEDTIMESTAMP BETWEEN DATEADD(DAY, -60, CAST(GETDATE() AS DATE)) AND CAST(GETDATE() AS DATE)
      AND a.WCHCONTAINERNUMBER NOT LIKE 'AIR%'
      AND a.WCHDESTINATION NOT IN ('100', '101', '12', '131', '01', '3', '990')
      AND SUBSTRING(LTRIM(RTRIM(a.WCHCONTAINERNUMBER)), 1, 4) NOT IN ('AAAR', 'AIIR', 'AAIR', 'AIRR', 'AIR_', 'AIR1', 'AAII', 'ARRR')
...
SELECT 
    ...
    CASE 
        WHEN LTRIM(RTRIM(SUBSTRING(h.WCHCONTAINERSIZE, 1, 2))) = '53' THEN d.Cubes / 3831
        WHEN LTRIM(RTRIM(SUBSTRING(h.WCHCONTAINERSIZE, 1, 2))) = '50' THEN d.Cubes / 3333
        WHEN LTRIM(RTRIM(SUBSTRING(h.WCHCONTAINERSIZE, 1, 3))) = '40H' THEN d.Cubes / 2650
        WHEN LTRIM(RTRIM(SUBSTRING(h.WCHCONTAINERSIZE, 1, 3))) = '40' THEN d.Cubes / 2383
        WHEN LTRIM(RTRIM(SUBSTRING(h.WCHCONTAINERSIZE, 1, 3))) = '45' THEN d.Cubes / 3058
        WHEN SUBSTRING(h.WCHCONTAINERSIZE, 1, 1) = '2' THEN d.Cubes / 1191
        ELSE d.Cubes / 2650 
    END AS Utilization,
    CASE 
