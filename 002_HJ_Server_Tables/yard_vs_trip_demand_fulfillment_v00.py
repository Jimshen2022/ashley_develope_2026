    ...
    for dem_idx, demand_row in demand_df.iterrows():
        ...
        if item_needed in supply_grouped.groups:
            ...
            for sup_idx in supply_indices:
                # 计算该供应行还剩多少可用数量
                available_qty = supply_available.loc[sup_idx, 'unreceived_qty'] - supply_available.loc[sup_idx, 'allocated_qty']
...
def process_demand_sheet(demand_df, allocations_df):
    ...
    alloc_summary = allocations_df.groupby(['trip_number', 'item_number']).agg(
        ...
        Allocated_Supply_Details=('container_number', lambda x: '; '.join(f"{row['container_number']} * {int(row['allocated_qty'])}" for _, row in allocations_df[allocations_df['container_number'].isin(x)].iterrows()))
    ).reset_index()
    
    # 修正Details的生成方式，使其更准确
    alloc_details = allocations_df.copy()
    alloc_details['detail_str'] = alloc_details.apply(lambda row: f"{row['container_number']} * {int(row['allocated_qty'])}", axis=1)
    details_summary = alloc_details.groupby(['trip_number', 'item_number'])['detail_str'].apply(lambda x: '; '.join(x)).reset_index(name='Allocated_Supply_Details')

    alloc_summary = pd.merge(alloc_summary.drop(columns=['Allocated_Supply_Details']), details_summary, on=['trip_number', 'item_number'], how='left')
    ...
    demand_final = pd.merge(demand_result, trip_summary[['trip_number', 'Allocated_Status', 'Fulfill_Rate']], on='trip_number', how='left')
...
def process_supply_sheet(supply_df, allocations_df):
    ...
    alloc_details['detail_str'] = alloc_details.apply(lambda row: f"{row['trip_number']} * {int(row['allocated_qty'])} * {row['dispatch_date_str']}", axis=1)
    
    details_summary = alloc_details.groupby(['container_number', 'item_number'])['detail_str'].apply(lambda x: '; '.join(x)).reset_index(name='Allocated_Demand_Details')
    qty_summary = alloc_details.groupby(['container_number', 'item_number'])['allocated_qty'].sum().reset_index(name='Allocated_Demand_Qty')
    ...
    supply_final = pd.merge(supply_result, container_summary[['container_number', 'Allocated_Status']], on='container_number', how='left')
...
def main():
    ...
    output_filename = f"Allocation_Result_{timestamp}.xlsx"
