...
SELECT DISTINCT
    ...
    CASE
        WHEN t3.ITCLS LIKE 'Z%' AND LEFT(t3.ITNBR,1) IN ('B','E','W','H','T')  THEN 'CG'
...
WITH TransactionBase AS (
    SELECT t1.*,
        ...
        -- Building
           CASE
               WHEN t1.tran_type in ('111') and t1.equipment_zone in ('B4UPH')  THEN 'B4'
               WHEN t1.tran_type in ('111') and t1.equipment_zone  IN ('B3ML4BD')  THEN 'B3'
               WHEN t1.tran_type in ('111') and t1.equipment_zone in ('B5UPH')  THEN 'B5'
               WHEN t1.tran_type in ('111')   THEN 'B4'
               ...
               WHEN t1.tran_type in ('374') and t1.equipment_zone in ('B4UPH')  THEN 'B4'
               WHEN t1.tran_type in ('374') and t1.equipment_zone  IN ('B3ML4BD')  THEN 'B3'
               WHEN t1.tran_type in ('374') and t1.equipment_zone in ('B5UPH')  THEN 'B5'
               WHEN t1.tran_type in ('374')   THEN 'B4'
               WHEN t1.tran_type = '368' AND t1.location_id_2 LIKE 'D3%' THEN 'B3'
...
SELECT t.*,
                ...
                where t.lot_number ='501605227247'
