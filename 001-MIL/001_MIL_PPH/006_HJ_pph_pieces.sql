    ...
    SELECT t1.*,
        ...
        -- Building
           CASE
               ...
               WHEN t1.tran_type in ('111') and LEFT(t1.item_number,1) IN ('T')  THEN 'B5'
               ...
               WHEN t1.tran_type = '374' AND LEFT(t1.wh_id_2,2) IN ('M4','BD') THEN 'B4'
               WHEN t1.tran_type = '374' AND LEFT(t1.wh_id_2,2) IN ('M5') THEN 'B5'
               WHEN t1.tran_type = '374' AND LEFT(t1.wh_id_2,2) IN ('QC') THEN 'B5'
               WHEN t1.tran_type = '374'  and LEFT(t1.item_number,1) IN ('B','E','W','H') AND LEFT(t1.wh_id_2,2) IN ('US','BD') THEN 'B4'
               WHEN t1.tran_type = '374'  and LEFT(t1.item_number,1) IN ('T') AND LEFT(t1.wh_id_2,2) IN ('US','BD') THEN 'B5'
               WHEN t1.tran_type = '374'  and LEFT(t1.item_number,1) IN ('M') AND LEFT(t1.wh_id_2,2) IN ('US','BD') THEN 'B3'
