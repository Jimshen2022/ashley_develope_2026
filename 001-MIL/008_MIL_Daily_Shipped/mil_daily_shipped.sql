...
WITH itm AS (
    SELECT t.STID, t.ITNBR, t.ITCLS, t.B2Z95S, t.WEGHT, s.ITMCQTY,
        CASE
            ...
            WHEN t.ITCLS IN ('CTA','MTA','HTA','GTA','CHS') THEN 'RP CGs'
            ...
            WHEN t.ITCLS IN ('ZABC','ZECD','ZDAA','ZDWC','ZDAB','ZDAE','ZDAW',
                              'ZDBC','ZDAY','ZEBR','ZVTY','ZVTC','ZEOT','ZDYB') THEN 'CaseGoods'
            WHEN t.ITCLS IN ('ZAIS','ZKIS','ZNFR','ZKBP','ZKBA','ZBMA','ZCIM','ZBMT') THEN 'Bedding'
            ELSE 'Check'
        END AS Product
...
ContainerDetails AS (
    ...
    SELECT
        ...
        a.WCIQUANTITYLOADED * c.B2Z95S AS Cubes,
        CEIL(a.WCIQUANTITYLOADED / c.ITMCQTY) AS Cartons,
        TRIM(a.WCIORIGIN) || '-' || TRIM(a.WCICONTAINERNUMBER) || '-' || TRIM(a.WCIDESTINATION)
            || '-' || SUBSTR(CHAR(a.WCIARCHIVETIMESTAMP), 1, 13) AS Container#,
        c.Product
...
HeaderDetails AS (
    SELECT
        ...
        a.WCHCONTAINERSIZE, a.WCHDOORNUMBER, a.WCHBUILDING, a.WCHPOSTEDTIMESTAMP,
    ...
    SELECT
        TRIM(a.WCHCONTAINERNUMBER) AS ContainerNumber,
        a.WCHCONTAINERSIZE, a.WCHDOORNUMBER, a.WCHBUILDING, a.WCHPOSTEDTIMESTAMP,
        TRIM(a.WCHORIGIN) || '-' || TRIM(a.WCHCONTAINERNUMBER) || '-' || TRIM(a.WCHDESTINATION)
            || '-' || SUBSTR(CHAR(a.WCHARCHIVETIMESTAMP), 1, 13) AS Container#,
        a.WCHTOTALCUBES AS H_Cubes
...
SELECT
    ...
    'MIL' AS WH,
    CASE
        WHEN ROW_NUMBER() OVER (PARTITION BY d.Container#, d.ItemNumber
                                ORDER BY d.WCILASTMAINTENANCETIMESTAMP) = 1
        THEN 1 ELSE 0
    END AS SKU_Count
