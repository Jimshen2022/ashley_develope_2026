WITH Exclude_Locations AS (
    -- Dynamic exclusion list string to be injected via Dataflow
    SELECT value AS LLOCN 
    FROM STRING_SPLIT('RS001,S01ST1,PIC01,LMF001', ',')
),
oh1 AS (
    -- Grouping snapshot data by Date to analyze historical trends
    SELECT
        a1.ITNBR,
        t2.ITDSC,
        t2.ITCLS,
        ...
        a1.HOUSE,
        a1.LLOCN,
        CAST(a1.SnapshotDate AS DATE) AS SnapshotDate, -- Cast to DATE to remove timestamps for cleaner grouping
...
SELECT
    ...
    -- Calculate days elapsed based on SnapshotDate, NOT the current GETDATE()
    CASE
    ...
    -- Evaluate the Day ranges against the SnapshotDate
    CASE
        WHEN lt.LAST_UPDDT IS NULL THEN NULL
    ...
    END                                                     AS "Day ranges"
FROM oh1
-- The OUTER APPLY function acts as a parameterized subquery executed for each row in oh1
OUTER APPLY (
    SELECT TOP 1
    ...
    WHERE T1.HOUSE = '51'
      ...
      -- Crucial filter: The transaction must have occurred ON or BEFORE the snapshot date
