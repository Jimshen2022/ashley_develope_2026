-- 设置参数值
DECLARE @in_load_id VARCHAR(30) = '%';

-- 用表变量存储多个 wh_id
DECLARE @in_wh_id TABLE (wh_id VARCHAR(10));
INSERT INTO @in_wh_id VALUES ('35'), ('31'), ('33'), ('36');
...
FROM (
    ...
    WHERE lok.source  = 't_order'
      ...
      -- ✅ 修复1：改用表变量做 IN 匹配
      AND orm.wh_id IN (SELECT wh_id FROM @in_wh_id)
      -- ✅ 修复2：load_id 通配符过滤
      AND orm.load_id LIKE @in_load_id
    GROUP BY orm.wh_id,
             orm.load_id,
             tlk.description,
             orm.order_number,
