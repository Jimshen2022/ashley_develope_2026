...
SELECT 
    ...
    CASE 
        ...
        THEN CAST(h.WCHPOSTEDTIMESTAMP AS DATE)  -- D班: 使用当天日期
        ELSE 
            CASE 
                WHEN CAST(h.WCHPOSTEDTIMESTAMP AS TIME) >= '19:00:00' 
                THEN CAST(h.WCHPOSTEDTIMESTAMP AS DATE)  -- N班 19:00-23:59: 使用当天日期
                ELSE DATEADD(DAY, -1, CAST(h.WCHPOSTEDTIMESTAMP AS DATE))  -- N班 00:00-06:59: 使用前一天日期
            END
    END AS Shift_Date,
    CASE 
