...
select t.week_saturday_date,
       ...
       SUM(t.Others_Quantity) AS Others_Quantity,
       SUM(t.Wanek_Quantity)+SUM(t.Others_Quantity) AS Total_Quantity
from trx as t
left join itm as i on t.item_number = i.ITNBR
where i.PICKPUT = 'UPH'
group by t.week_saturday_date
order by t.week_saturday_date;