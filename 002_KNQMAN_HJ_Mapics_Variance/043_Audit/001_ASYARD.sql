...
LEFT JOIN (SELECT a4.trailer_id, a4.carrier_id, a4.equipment_id, a4.wh_id, a4.entered_yard, a4.status, a4.location_id
           FROM trailer AS a4
...
SELECT * FROM Distribution_Warehouse_Wholesale.t_asn as a1 WHERE a1.wh_id = '335' and a1.status in ('NEW','CHECKED IN')
SELECT * FROM Distribution_Warehouse_Wholesale.ASN_Detail AS a2 where a2.wh_id = '335'
...
SELECT *
			FROM Distribution_Warehouse_Wholesale.ASN_Detail AS a2 where a2.wh_id = '335'




SELECT a4.trailer_id, a4.carrier_id, a4.equipment_id, a4.wh_id, a4.entered_yard,a4.status, a4.location_id
           FROM Distribution_Warehouse_Wholesale.Trailer  AS a4
           WHERE a4.Wh_id = '335' and a4.status in ('IN DOOR','IN YARD CHASSIS')
           Group by a4.trailer_id, a4.carrier_id, a4.equipment_id, a4.wh_id, a4.entered_yard,a4.status,a4.location_id


SELECT * FROM Distribution_Warehouse_Wholesale.Trailer  AS a4 WHERE a4.Wh_id = '335' and a4.status in ('IN DOOR','IN YARD CHASSIS')
					  order by t4.trailer_id
					  and t4.carrier_trailer_number like 'HMMU695920%'
