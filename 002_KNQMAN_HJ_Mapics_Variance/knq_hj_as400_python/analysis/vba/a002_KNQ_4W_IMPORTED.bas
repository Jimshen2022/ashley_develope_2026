Attribute VB_Name = "a002_KNQ_4W_IMPORTED"
Sub a002_KNQ_4W_IMPORTED_()
    ...
    Set wb = GetObject("C:\Users\" & user_name & "\Downloads\KNQ_4W_IMPORTED.xlsx")
    arr = wb.ActiveSheet.[a1].CurrentRegion
    wb.Close False
    
    For i = 2 To UBound(arr)
        arr(i, 10) = CStr(Mid(arr(i, 10), 4, 2) & "/" & Mid(arr(i, 10), 1, 2) & "/" & Mid(arr(i, 10), 7, 4))
        arr(i, 6) = CStr(Trim(arr(i, 6)))
        arr(i, 11) = CStr(Trim(arr(i, 11)))
    Next
