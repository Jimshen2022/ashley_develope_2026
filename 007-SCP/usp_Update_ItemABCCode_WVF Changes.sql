-- Changes found on lines 53-63 and lines 269-323



/****** Object:  StoredProcedure [SupplyChain_Enh].[usp_Update_ItemABCCode_WVF]    Script Date: 7/29/2020 10:08:58 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROC [SupplyChain_Enh].[usp_Update_ItemABCCode_WVF] AS
...
SET @String = 'SupplyChain_Enh.usp_Update_ItemABCCode_WVF'
...
INSERT INTO SupplyChain_Enh.ItemABCCode_WVF (
		[Item SKU],
		[Current Status],
		[Future Status],
...
/*  Original Logic using the @MaxX and @MaxY hard coded parameters                      
           [XYZ Code] = CASE
                            WHEN ABC.[Manufacturing Status] = 'Discontinued' THEN
                                'Z'
                            ...
                            WHEN ABC.CoefVar <= @MaxX THEN
                                'X'
                            WHEN ABC.CoefVar > @MaxX
                                 AND ABC.CoefVar <= @MaxY THEN
                                'Y'
                            ELSE
                                'Z'
                        END
...
VALUES ('SupplyChain_Enh' , 'ItemABCCode_WVF', @DateValue )
...
DROP TABLE #XYZ;
IF OBJECT_ID('tempdb..#CumlPerc') IS NOT NULL
DROP TABLE #CumlPerc;
END
