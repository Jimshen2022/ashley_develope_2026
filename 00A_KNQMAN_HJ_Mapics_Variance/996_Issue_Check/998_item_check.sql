  --select * from t_hu_master where location_id= 'A3017KA2' 
  ...
  --select * from t_work_q where location_id  LIKE 'A3017KA2%' 

  --select top 10 *  from  t_stored_item where location_id = 'A3017KA2' 
...
select top 10 *  from  t_location where location_id like 'A3020H%1'; 
select top 10 *  from  t_fwd_pick
select top 10 *  from  t_zone_loca 
select top 10 *  from  t_stored_item 
select  *  from  t_class_loca where location_id like 'A302%' and class_id like 'UPHM%'

select top 100 * from t_item_uom where item_number like 'W%' AND pick_put_id = 'PALLT'
*/

-- 先创建 pallet capacity 的 CTE
--select top 10 *  from  t_class_loca where location_id like 'A3018H%1'; 
--select top 10 *  from  t_location where location_id like 'A3018CA1%'; 
--select top 10 *  from  t_loc_pallet_capacity  where location_id like 'A3018%';
...
sto as (
select item_number,location_id, sum(actual_qty) as onhand  from t_stored_item where wh_id = '335' and location_id like 'A3%' group by item_number, location_id
)
...
SELECT 
    ...
    STUFF((
        SELECT DISTINCT ',' + t3.zone
        FROM t_zone_loca t3
        WHERE t3.wh_id = t0.wh_id 
            AND t3.location_id = t0.location_id
        FOR XML PATH('')
    ), 1, 1, '') AS zones,
    STUFF((
        SELECT DISTINCT ',' + t4.item_number + '*' + CAST(t4.actual_qty AS VARCHAR)
        FROM t_stored_item t4
        WHERE t4.location_id = t0.location_id 
        FOR XML PATH('')
    ), 1, 1, '') AS onhand,
...
LEFT JOIN zone z ON t0.location_id = z.location_id
LEFT JOIN sto s ON s.location_id = t0.location_id 
WHERE 1=1
  AND t0.location_id LIKE 'A3017KA2%' 
   
  --or t0.location_id like 'A3018[DFHKMPRTWY]%1'
  --AND t0.location_id LIKE 'A3018[CEGJLNQSUXZ]%'
