...
WHERE t1.lot_number = '503952069729'
...
select * 
from Distribution_Warehouse_Wholesale.employee as e
where e.employee_id in ('50771', '30008')
...
WHERE t1.wh_id = '335'
	AND t1.tran_type in ('151','951')
-- 	AND t1.control_number_2 like '0039312%'
    AND t1.item_number IN ('D388-124')
    AND t1.start_tran_date >= '2026-02-01'
GROUP by  t1.start_tran_date,t1.item_number,t1.control_number_2,t1.lot_number
order by t1.item_number, t1.start_tran_date


-- RP received by item
SELECT t1.start_tran_date,t1.item_number,t1.control_number_2, sum(case when t1.tran_type = '951' then -t1.tran_qty else t1.tran_qty end) as qty
FROM t_tran_log AS t1
WHERE t1.wh_id = '335'
	AND t1.tran_type in ('151','951')
	AND t1.control_number_2 like 'P2SD358%'
    -- AND t1.item_number IN ('D388-124')
    AND t1.start_tran_date >= '2025-10-30'
GROUP by  t1.start_tran_date,t1.item_number,t1.control_number_2
order by t1.item_number, t1.start_tran_date
...
-- RP received by item
SELECT t1.start_tran_date,t1.control_number_2, sum(case when t1.tran_type = '951' then -t1.tran_qty else t1.tran_qty end) as qty
FROM t_tran_log AS t1
WHERE t1.wh_id = '335'
	AND t1.tran_type in ('151','951')
	AND t1.control_number_2 like 'P2SD358%'
    -- AND t1.item_number IN ('D388-124')
    AND t1.start_tran_date >= '2025-10-30'
GROUP by  t1.start_tran_date,t1.control_number_2
...
-- Orphaned SN current status
 ...
 where t.wh_id = '335'
    AND t.serial_number IN ('635093937556') order by t.serial_no_status, t.status_change

-- SN trx
SELECT t1.tran_type, t1.description, t1.start_tran_date, cast(t1.start_tran_time as 'hh:mm:ss') as start_tran_time, t1.employee_id, t1.control_number_2, t1.wh_id, t1.item_number, t1.lot_number, t1.location_id, t1.location_id_2, t1.tran_qty, t1.routing_code
FROM Distribution_Warehouse_Wholesale.TranLog AS t1
WHERE 
    t1.lot_number IN ('635093937556')
    AND t1.start_tran_date >= '2025-01-01'
