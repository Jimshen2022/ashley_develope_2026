...
# 读取 CSV 文件
df = pd.read_csv(r'C:\Users\jishen\Downloads\INPUT_LIST.csv')
...
for i in range(total_batches):
    ...
    chunk['RP QTY'] = chunk['RP QTY'].astype(str).str.strip()  # 去掉前后空格
    chunk['RP QTY'] = chunk['RP QTY'].str.replace(',', '')  # 去掉千分位
    chunk['RP QTY'] = chunk['RP QTY'].str.extract(r'(\d+)', expand=False)  # 提取纯数字
    body = ""
