...
try:
    ...
    # 创建数据透视表：行为 site 和 product，列为 week_ending_saturday，值为 tran_qty 的总和
    summary = pd.pivot_table(
        df,
        values='tran_qty',
        index=['site', 'product'],
        columns='week_ending_saturday',
        aggfunc='sum',
        fill_value=0
    )

    # 重置索引使 site 和 product 成为普通列
...
try:
    ...
    print(f"- Summary sheet: 按 Site 和 Product 汇总的数据透视表")
