...
WHERE
    -- 只保留未匹配的记录: ASN Only 或 TranLog Only
    NOT (a.serial_number IS NOT NULL AND t.lot_number IS NOT NULL)
ORDER BY
    Match_Status,
...
# --- Helper: format serial number columns as text for Excel ---
...
if __name__ == "__main__":
    ...
    # 2. Execute SQL and fetch results
    try:
        print("Executing SQL query (this handles all joins and SN expansion in SQL Server)...")
        df = pd.read_sql(SQL_QUERY, engine)
        print(f"Query complete. Total rows fetched: {len(df)}")
    except Exception as e:
        print(f"Query failed: {e}")
        sys.exit()

    # 3. Format serial number columns so Excel treats them as text
    for col in ['serial_number', 'serial_number_start', 'serial_number_end', 'tl_lot_number']:
    ...
    filename  = f"ashton_received_sn_vs_asn_v4_{current_time}.csv"
