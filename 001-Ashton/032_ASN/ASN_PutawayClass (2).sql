...
asd as (
    select 
        ...
        sum(quantity_shipped) - sum(quantity_received) as remaining_quantity,
        STRING_AGG(CAST(serial_number_start AS VARCHAR) + '-' + CAST(serial_number_end AS VARCHAR), ', ') 
            WITHIN GROUP (ORDER BY serial_number_start ASC) as Serial_Numbers
    from t_asn_detail
...
select 
    ...
    uom,
    -- 新增的简单分类列
    CASE WHEN pick_put_id = 'UPH' THEN 'UPH' ELSE 'CG' END as product_category,
    width,
    height,
    depth,
    SCOOP_Qty,
