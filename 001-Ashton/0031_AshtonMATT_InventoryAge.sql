-- Sep.23.2024, Ashton mattress inventory age  -- JimShen
-- Sep.23.2024, Ashton mattress inventory age  -- JimShen
...
WITH r1 AS (
    ...
    WHERE t1.wh_id IN ('335') AND
          t1.start_tran_date >= '2022-01-01' AND t1.lot_number is not null and len(t1.lot_number)>5
...
FROM Distribution_Warehouse_Wholesale.TranLog AS t1
WHERE t1.wh_id IN ('335') AND t1.start_tran_date >= '2022-01-01' AND t1.lot_number is not null and len(t1.lot_number)>5
...
SELECT t1.wh_id
	...
	, t1.location_id
	, CASE
	        when t1.location_id LIKE 'A3%' THEN 'Asthon Racking location'
	    	when t1.location_id LIKE 'D%' THEN 'Asthon Door location'
	    	when t1.location_id LIKE 'S%' THEN 'Asthon Shipping Stage location'
	    	when t1.location_id LIKE 'RS%' THEN 'Asthon Receiving Stage location'
...
WHERE  t1.wh_id  IN ('335')
	AND T1.item_number LIKE 'M%' AND t1.serial_number is not null and len(t1.serial_number)>5 and len(t1.item_number) >6
