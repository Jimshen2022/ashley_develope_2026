...
SELECT
    ...
    i1.product,
    CASE
        WHEN t1.tran_type IN ('151', '183', '951')
            THEN CONCAT(t1.control_number, '_', t1.hu_id_2)
        WHEN t1.tran_type = '322'
            THEN (
    ...
    SUM(CASE
        WHEN t1.tran_type IN ('322') THEN t1.tran_qty ELSE 0 END) AS Loaded_Qty,
    SUM(CASE
        WHEN t1.tran_type IN ('322') THEN t1.tran_qty*i1.unit_volume ELSE 0 END) AS Loaded_cubes
FROM (select t.start_tran_date,
       ...
       t.routing_code,
       t.tran_qty
      from Distribution_Warehouse_Wholesale.TranLog as t where t.wh_id IN ('335')  AND t.start_tran_date >= '2024-01-01'
      AND t.tran_type IN ('322', '151', '183', '951'))  AS t1
...
--     END AS INT) as Avg_Cube_per_Container_Received,
    SUM(CASE WHEN a1.tran_type IN ('322') and a1.row_num_shipping = 1 then 1 else 0 end) as Loaded_Container_Count,
    SUM(a1.Loaded_Qty) as Loaded_Qty,
    CAST(SUM(a1.Loaded_cubes) AS INT) as Loaded_cubes
--     CAST(CASE
--         WHEN NULLIF(SUM(CASE WHEN a1.tran_type IN ('322') and a1.row_num_shipping = 1 then 1 else 0 end), 0) IS NULL THEN NULL
--         ELSE SUM(a1.Loaded_cubes)/NULLIF(SUM(CASE WHEN a1.tran_type IN ('347') and a1.row_num_shipping = 1 then 1 else 0 end), 0)
