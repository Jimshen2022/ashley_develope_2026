...
SELECT
    ...
    a1.OrderType4,
    -- SEL/SCH column:
    --   V = Tripped  → found in AFILELIB.BOLITEM  (order line sent to WMS for shipping)
    --   E = Scheduled → found in AFILELIB.ORDSCHD (order line scheduled, not yet sent to WMS)
    --   (blank) = neither scheduled nor tripped
    CASE
        WHEN bol.BORDNO IS NOT NULL THEN 'V'
        WHEN sch.SCORDNO IS NOT NULL THEN 'E'
        ELSE ''
    END AS "SEL/SCH"
...
LEFT JOIN (
    SELECT DISTINCT SCCUST#, SCORDNO, SCITMNO, SCITMSQ
...
LEFT JOIN (
    SELECT DISTINCT BCUST#, BORDNO, BSEQ#, BITEM#
