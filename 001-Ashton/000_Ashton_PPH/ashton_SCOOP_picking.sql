...
DECLARE @EndDate DATETIME = '2025-06-04';

SET @wh_id_list = '335,335';
SET @tran_list = '363,363';
    ...
    SELECT t.*,
       CASE
        ...
        CASE
            WHEN t.pph_type in ('Picking-SCOOP')  then
                ROW_NUMBER() OVER (
                    PARTITION BY t.start_tran_date, t.item_number
                    ORDER BY t.trx_date_time
                )
            ELSE 0
        END as sku_num
...
SELECT
   t9.*,
    ...
    CASE
        ...
        ELSE 0
    END AS pieces,
    CASE WHEN t9.sku_num = 1 THEN 1 ELSE 0 END AS daily_sku_count,
...
FROM trx_2 AS t9
WHere t9.pph_type = 'Picking-SCOOP'
