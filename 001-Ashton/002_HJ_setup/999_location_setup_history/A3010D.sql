...
select top 10 *  from  t_loc_pallet_capacity where location_id like 'A3020C%1'
...
--select top 10 *  from  t_class_loca where location_id like 'A3020H%1'; 
--select top 10 *  from  t_loc_pallet_capacity  where location_id = 'A3020HA1';
...
SELECT 
    ...
    SUBSTRING(t0.location_id,8,1) AS tier,
    t0.type,
    t0.status,
    STUFF((
...
WHERE 1=1
  AND SUBSTRING(t0.location_id,1,5) IN ('A3012')
  --AND SUBSTRING(t0.location_id, 6, 1) IN ('D','F','H','K','M','P','R','T')
  --AND SUBSTRING(t0.location_id, 6, 1) IN ('C','E','G','J','L','N','Q','S') 
