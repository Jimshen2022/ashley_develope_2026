WITH sto AS
 ...
 itemmaster AS (SELECT i.ITNBR,
                       ...
                       CASE
                           ...
                           WHEN SUBSTRING(i.ITNBR, 1, 4) IN ('100-') THEN 'CG'
                           WHEN SUBSTRING(i.ITNBR, 1, 1) IN ('1', '2', '3', '4', '5', '6', '7', '8', '9', 'U')
                               THEN 'UPH'
                           WHEN SUBSTRING(i.ITNBR, 1, 1) IN ('A') AND i.B2Z95S * 0.028317 <= 0.4 THEN 'ACCESSORY'
                           WHEN SUBSTRING(i.ITNBR, 1, 1) IN ('L', 'Q', 'R') THEN 'ACCESSORY'
                           WHEN SUBSTRING(i.ITNBR, 1, 1) IN ('M') AND LEN(i.ITNBR) = 6 THEN 'ACCESSORY'
...
/*SELECT t4.ITNBR, t2.HOUSE, t2.MOHTQ, t2.WHSLC, t2.ITCLS, t2.QTSYR, t4.B2Z95S,
...
t3.ITNBR = t4.ITNBR AND t4.STID = t1.HOUSE AND t1.HOUSE='335'*/
    ),
Loc AS (
    SELECT *
...
SELECT
    ...
    CASE WHEN i.product = 'UPH' and i.Unit_CBM = 0 THEN  s.actual_qty* 46.828 * 0.028317    -- 46.828 is Phumy UPH average unit ft3
         WHEN i.product = 'ACCESSORY' and i.Unit_CBM = 0 THEN  s.actual_qty * 0.01          --  0.1 is Phumy accessory average unit CBM
