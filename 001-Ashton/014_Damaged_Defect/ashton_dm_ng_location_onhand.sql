...
sn_in_damaged_loc as (
    select distinct
        lot_number,
        item_number,
        convert(varchar(19), last_datetime, 120) as scanned_into_damaged_loc_datetime,
        last_tran_type
    from (
        select 
            lg.lot_number,
            lg.item_number,
            cast(lg.start_tran_date + lg.start_tran_time as datetime) as tran_datetime,
            lg.tran_type,
            max(cast(lg.start_tran_date + lg.start_tran_time as datetime)) 
                over (partition by lg.lot_number, lg.item_number) as last_datetime,
            first_value(lg.tran_type) 
                over (partition by lg.lot_number, lg.item_number 
                      order by cast(lg.start_tran_date + lg.start_tran_time as datetime) desc) 
                as last_tran_type
