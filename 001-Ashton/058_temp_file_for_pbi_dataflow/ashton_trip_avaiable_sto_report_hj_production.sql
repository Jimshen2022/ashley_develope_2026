...
/* Trip Available STO report — direct query version
   Filters fixed to:
     - Search Type = 'ALL' (All Items)
     - FP Location Only? = 'N' (No; include all buildings, not only Forward Pick)
   Instructions:
     - Set @in_vchWhID, @in_vchDispatchStartDate, @in_vchDispatchEndDate as needed.
     - @in_Confirmed = 'A' means include both confirmed and unconfirmed dispatches.
*/
DECLARE @in_vchWhID NVARCHAR(10) = N'335';
DECLARE @today_midnight DATETIME = DATEADD(DAY, DATEDIFF(DAY, 0, GETDATE()), 0);
DECLARE @in_vchDispatchStartDate DATETIME = DATEADD(DAY, -7, @today_midnight);
DECLARE @in_vchDispatchEndDate   DATETIME = DATEADD(SECOND, -1, DATEADD(DAY, 30, @today_midnight));
DECLARE @in_vchType NVARCHAR(20) = N'ALL';  -- All Items
DECLARE @in_vchFWP  NVARCHAR(2)  = N'N';    -- FP Location Only? = No
DECLARE @in_Report  VARCHAR(1)   = 'D';
DECLARE @in_Confirmed VARCHAR(1) = 'A';
