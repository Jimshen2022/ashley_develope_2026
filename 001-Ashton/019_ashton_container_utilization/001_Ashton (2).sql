...
WITH MaxEnteredYard AS (
    ...
    WHERE wh_id = '335'
        AND entered_yard >= '2024-12-10 07:00:00'
        AND entered_yard < DATEADD(HOUR, 7, CAST(CAST(GETDATE() + 1 AS DATE) AS DATETIME))
...
ctn AS (
    SELECT DISTINCT 
        ...
        CASE 
            ...
            ELSE '40H'
        END AS Ctn_size
    FROM Distribution_Warehouse_Wholesale.Trailer t1
    INNER JOIN MaxEnteredYard mey 
        ON t1.equipment_id = mey.equipment_id 
        AND t1.entered_yard = mey.max_entered_yard
    WHERE t1.wh_id = '335'
        AND t1.entered_yard >= '2025-01-01 07:00:00'
        AND t1.entered_yard < DATEADD(HOUR, 7, CAST(CAST(GETDATE() + 1 AS DATE) AS DATETIME))
),
i AS (
    SELECT 
        T1.STID,
...
tp AS (
    ...
    WHERE t1.wh_id IN ('335')
        AND t1.start_tran_date >= '2025-01-01 07:00:00'
        AND t1.start_tran_date < DATEADD(HOUR, 7, CAST(CAST(GETDATE() + 1 AS DATE) AS DATETIME))
...
tjc AS (
    ...
    WHERE T1.wh_id IN ('335')
        AND T1.bill_number IS NOT NULL
        AND T1.dispatch_date >= '2025-01-01 07:00:00'
        AND T1.dispatch_date < DATEADD(HOUR, 7, CAST(CAST(GETDATE() + 1 AS DATE) AS DATETIME))
        AND t1.shipment_status IN ('Shipped')
        AND T1.trailer_number IS NOT NULL
)
-------------------------------------------MAIN-------------------------------------------------------------------
SELECT 
    t1.tran_type,
    ...
    tjc.ctn_size_2,
    CASE 
        WHEN tp.Ctn_size IS NOT NULL THEN tp.Ctn_size 
        ELSE tjc.ctn_size_2 
    END AS Container_size_final,
...
GROUP BY 
    ...
    tjc.ctn_size_2,
    CASE 
        WHEN tjc.ctn_size_2 IS NOT NULL THEN tjc.ctn_size_2 
        ELSE tp.Ctn_size 
    END,
