...
LEFT JOIN b as b ON b.BokTripNumBer = t1.CAST(SUBSTRING(t1.control_number_2,1,CHARINDEX('-',t1.control_number_2) -1) AS INT) and b.BokContainerNumBer = t1.routing_code
WHERE t1.wh_id IN ('335') AND t1.start_tran_date >= DATEADD(DAY,-60,GETDATE()) AND t1.tran_type IN ('347')
