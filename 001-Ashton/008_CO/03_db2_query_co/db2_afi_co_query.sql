...
WITH BaseData AS (
    SELECT
        ...
        t6.ITWHSE, t6.ITCSNO, t6.ITSPNO, T5.XNTRPN, T5.XNINVR, T5.XNORNO, T5.XOTYP1 AS TYPE1, T5.XOTYP2 AS TYPE2, T5.XOTYP3 AS TYPE3,T5.XOTYP4 AS TYPE4,
        t1.STACD,
    ...
    WHERE
       ...
       -- AND t5.XNTRPN <> 0
        ...
        AND t4.INIVDT BETWEEN 20260324 AND INTEGER(REPLACE(CHAR(CURRENT DATE), '-', ''))
        AND t6.ITSHQT > 0
),
ContainerTypes AS (
    SELECT
...
SELECT  bd.INIVDT, bd.ITITNO, bd.INWHSE, bd.ITITCL, bd.ITSHQT, bd.UUCCIM, bd.XCUS#,
    ...
    bd.TYPE1, bd.TYPE2, bd.TYPE3, bd.TYPE4
FROM
    BaseData bd
