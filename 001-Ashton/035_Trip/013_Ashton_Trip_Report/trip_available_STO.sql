/*
Parameterized header — SAFE for SQL Server (avoids DATE type in DATEADD SECOND).
Drop this block at the top of your script, replacing your previous parameter declarations.
It sets a 7‑day window ending today 23:59:59 (server local time).

Key fixes:
- DO NOT use CAST(GETDATE() AS DATE) with DATEADD(SECOND,...).
- Use datetime arithmetic based on DATEDIFF/DAY anchor (0 = 1900‑01‑01) to get midnight safely.
- All date variables are DATETIME.
*/

-- Warehouse (edit as needed)
DECLARE @in_vchWhID NVARCHAR(10) = N'335';

DECLARE @now DATETIME = GETDATE();
-- Start = 当前时间往前 31 天（保留时分秒）
DECLARE @in_vchDispatchStartDate DATETIME = DATEADD(DAY, -31, @now);

-- End = 今天往后 31 天的日期的 23:59:59
DECLARE @in_vchDispatchEndDate DATETIME =
    DATEADD(SECOND, -1,
        DATEADD(DAY, DATEDIFF(DAY, 0, DATEADD(DAY, 31, @now)) + 1, 0)
    );

-- Fixed conditions per your requirement
DECLARE @in_vchType NVARCHAR(20) = N'ALL';  -- Search Type = All Items
DECLARE @in_vchFWP  NVARCHAR(2)  = N'N';    -- FP Location Only? = No

-- Optional placeholders retained for compatibility (adjust if your downstream logic uses them)
DECLARE @in_Report    VARCHAR(1) = 'D';
DECLARE @in_Confirmed VARCHAR(1) = 'A';     -- A=All, Y=Confirmed only, N=Unconfirmed only
