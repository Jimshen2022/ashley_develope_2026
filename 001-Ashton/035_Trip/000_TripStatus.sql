...
(
select
    LEFT(t.load_id, 7) as trip_nbr,  -- 1. 取 load_id 前7码作为新列
    STRING_AGG(t.load_id, ', ') as combined_load_id, -- 2. 将原始 load_id 用逗号连接
    ...
    t.trip_type_id
from t_load_master t
group by
    LEFT(t.load_id, 7), -- 3. Group By 必须包含这个新逻辑
    -- 4. 其他所有 SELECT 中出现的非聚合列，都必须放在 GROUP BY 中
    t.status,
    t.shipment_status,
    t.trip_create_date + t.trip_create_time, -- 注意：这里要和 Select 中的逻辑保持一致
